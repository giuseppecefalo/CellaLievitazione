...
#define BACKLIGHT_PIN 21           // Cambia con il tuo pin effettivo
#define INACTIVITY_TIMEOUT 50000   // Tempo di inattività in millisecondi (es. 10s)
...
int giorno_corrente = 0;  // 0 = Lunedì, 6 = Domenica
...
/* Funzione per l'aggiornamento della luminosità del display quando c'è attività*/
...
/* Controllo inattività per lo spegnimento del dispaly */
...
void my_touch_read(lv_indev_t *indev_drv, lv_indev_data_t *data) {
  ...
  if (data->state == LV_INDEV_STATE_PR) {
    user_activity_detected(); // attività rilevata
  }
...
void scanWiFi() {
  ...
  WiFi.mode(WIFI_STA);
  WiFi.disconnect(true, true);   // più robusto di disconnect()
  delay(200);
...
void connectToWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // Salva solo se ha senso (es. vuoi persistere anche l’ultima connessione ok)
...
void updateClockTask(lv_timer_t *timer) {
  ...
  // Se il WiFi è disponibile, prova a sincronizzare l'orario con NTP
  if (wifiAvailable) {
      if (!getLocalTime(&timeinfo)) {
          Serial.println("❌ Errore nel recupero dell'ora NTP! Uso RTC interno.");
      }
  } else {
      Serial.println("⚠️ WiFi non disponibile. Uso RTC interno.");
      time_t now;
  ...
  // Buffer per data completa: "Lunedì, 08/04/2024 - 14:35"
  char dataOraStr[64];
  ...
  Serial.printf("✅ Orologio aggiornato: %s\n", dataOraStr);  // Debug

  // Verifica se il puntatore è NULL
  if (objects.id_clock_label == NULL || objects.id_clock_label_2 == NULL) {
      Serial.println("⚠️ Errore: label non inizializzata! Reset evitato.");
      return;
  }
...
extern "C" {
  ...
  void gestisciAccensioneCaldaia(lv_timer_t* timer) {
    struct tm timeinfo;
    if (!getLocalTime(&timeinfo)) {
        Serial.println("❌ Errore nel recupero dell'orario - Caldaia spenta");
        digitalWrite(PINCALDAIA, LOW);
        lv_label_set_text(objects.programmazione, "Errore orario");
        return;
    }

    int giorno = (timeinfo.tm_wday + 6) % 7;  // Lunedì = 0, Domenica = 6
    ...
    if (programAccensione) {
        ...
        if (giorno >= 0 && giorno < 7 && slot >= 0 && slot < 48) {
            ...
            if (slotAttivo && tempC <= tMax) {
                ...
                Serial.println("🔥 Slot attivo e temperatura sotto tMax → Caldaia ACCESA");
              ...
              //  lv_obj_add_flag(objects.fiamma, LV_OBJ_FLAG_HIDDEN);    // Rendi invisibile l'oggetto Fiamma
                Serial.println("💤 Slot non attivo o temperatura sopra tMax → Caldaia SPENTA");
            }

            lv_label_set_text(objects.programmazione, slotAttivo ? "Programmazione ON" : "Programmazione OFF");
        } else {
            Serial.println("⚠️ Indici fuori range - Caldaia spenta per sicurezza");
        ...
        if (tempC < tMax) {
          accensioneCaldaia = true;
          Serial.println("🔥 Temperatura sotto tMin → Caldaia ACCESA (Manuale)");
        //  lv_obj_clear_flag(objects.fiamma, LV_OBJ_FLAG_HIDDEN);  // Mostra la fiamma
        }
        else {
          accensioneCaldaia = false;
          Serial.println("📴 Temperatura sopra tMax → Caldaia SPENTA (Manuale)");
...
extern "C" void salva_programmazione() {
  ...
  Serial.printf("✅ Salvataggio completato. SSID_len=%u PWD_len=%u\n", ssid_len, pwd_len);
}
  
// nuova funzione di connessione alla rete WiFi
// Questa funzione è richiamata da action.c
...
extern "C" void connect_to_wifi(const char* ssid, const char* password)
{
    ...
    if (WiFi.status() == WL_CONNECTED) {
        // ✅ copia credenziali “buone” nelle globali prima di salvare
...
extern "C" void carica_programmazione() {
  ...
  if (!file) {
    Serial.println("⚠️ File non trovato, caricamento saltato.");
    ssid_attivo[0] = '\0';
    password_attiva[0] = '\0';
    return;
  }
  ...
  if (file.read((uint8_t*)&tMin, sizeof(float)) != sizeof(float) ||
    ...
    Serial.println("❌ Errore lettura tMin/tMax (file corto o corrotto)");
    file.close();
    ssid_attivo[0] = '\0';
    password_attiva[0] = '\0';
    return;
  ...
  for (int giorno = 0; giorno < 7; giorno++) {
    ...
    if (file.read(buffer, 6) != 6) {
      Serial.printf("❌ Errore lettura giorno %d\n", giorno);
      file.close();
      ssid_attivo[0] = '\0';
      password_attiva[0] = '\0';
      return;
  ...
  if (file.available() > 0) {
   ...
   // Password
    ...
    Serial.printf("✅ Credenziali caricate. SSID='%s' pwd_len_file=%u pwd_len_real=%u\n",
                    ssid_attivo, pwd_len, (unsigned)real_len);

    } else {
    Serial.println("⚠️ Password non valida o mancante nel file (ignoro credenziali).");
    ssid_attivo[0] = '\0';
    password_attiva[0] = '\0';
    }
    
    } else {
      Serial.println("⚠️ SSID non valido o mancante nel file (ignoro credenziali).");
    }
  } else {
    Serial.println("ℹ️ Nessuna credenziale WiFi nel file (formato vecchio).");
  }

  file.close();

  Serial.printf("✅ Programmazione caricata. tMin: %.2f°C, tMax: %.2f°C\n", tMin, tMax);
}
...
void init_temp_rollers() {
  ...
  Serial.printf("✅ Roller impostati: tMin %.1f → %d, tMax %.1f → %d\n", tMin, index_min, tMax, index_max);
}
*/
...
void debug_dump_programmazione_e_blocca() {
  ...
  if (!LittleFS.exists("/programmazione.bin")) {
    Serial.println("❌ /programmazione.bin NON esiste");
  ...
  Serial.println("⛔ Programma bloccato per debug.");
...
//    carica_programmazione();  // ✅ Caricamento dei dati salvati
    ...
    // Errore nella gestione del file. sostituito dal codice che segue
...
//    debug_dump_programmazione_e_blocca();  // Debug del file di porgrammazione, stampa e blocca il programma
    ...
    Serial.println("❌ LittleFS KO: salto carica_programmazione e blocco i salvataggi.");
    }
...
// setupWiFiDebugEvents();
  ...
  //  aggiorna_button_program_page(); // aggiorna i button 
   ...
   // Imposta il GPIO come accensione caldaia
    ...
    // Imposta la modalità di connessione al WiFi - IMPORTANTE, siccome viene invocata una funzione LVGL, è importante che la funzione venga chiamata dopo l'inizializzazione della UI
    WiFi.mode(WIFI_STA);  // Mantiene la modalità STA per ESP-NOW e verifica se è istallato correttamente
...
//  lv_timer_handler(); // Aggiorna LVGL
  check_inactivity();  // Controlla inattività
